...
void _expose(const gx_lunaUI * const ui) {
...
void controller_expose(const gx_lunaUI * const ui,  const gx_controller * const control) {
	cairo_push_group (ui->cr);
...
void resize_event(gx_lunaUI * const ui) {
...
bool get_active_ctl_num(gx_lunaUI * const ui, int *num) {
	bool ret = false;
	for (int i=0;i<CONTROLS;i++) {
...
void scroll_event(gx_lunaUI * const ui, int direction) {
	float value;
	int num;
...
void button1_event(gx_lunaUI * const ui, double* start_value) {
	int num;
...
void motion_event(gx_lunaUI * const ui, double start_value, int m_y) {
...
void set_key_value(gx_lunaUI * const ui, int set_value) {
	float value = 0.0;
	int num;
	if (get_active_controller_num(ui, &num)) {
...
void key_event(gx_lunaUI * const ui, int direction) {
	float value;
	int num;
	if (get_active_controller_num(ui, &num)) {
		value = min(ui->controls[num].adj.max_value,max(ui->controls[num].adj.min_value, 
...
void set_previous_controller_active(gx_lunaUI * const ui) {
	int num;
	if (get_active_controller_num(ui, &num)) {
...
void set_next_controller_active(gx_lunaUI * const ui) {
	int num;
	if (get_active_controller_num(ui, &num)) {
		ui->controls[num].is_active = false;
		send_controller_event(ui, num);
...
void get_last_active_controller(gx_lunaUI * const ui, bool set) {
	int num;
	if (get_active_controller_num(ui, &num)) {
