...
static void resize_event(gx_lunaUI * const ui);
...
cairo_surface_t *cairo_image_surface_create_from_stream (gx_lunaUI * const ui, const unsigned char* name) {
...
static LV2UI_Handle instantiate(const struct _LV2UI_Descriptor * descriptor,
	...
	gx_lunaUI* ui = (gx_lunaUI*)malloc(sizeof(gx_lunaUI));
...
static void cleanup(LV2UI_Handle handle) {
	gx_lunaUI * const ui = (gx_lunaUI*)handle;
	cairo_destroy(ui->cr);
...
static void knob_expose(const gx_lunaUI * const ui, const gx_controller * const knob) {
...
static void bypass_expose(const gx_lunaUI * const ui,  const gx_controller * const switch_) {
	cairo_set_operator(ui->crf,CAIRO_OPERATOR_CLEAR);
	cairo_paint(ui->crf);
	cairo_set_operator(ui->crf,CAIRO_OPERATOR_OVER);
...
static void draw_controller(const gx_lunaUI * const ui,  const gx_controller * const controller) {
	if (controller->type < BSWITCH) knob_expose(ui, controller);
	else bypass_expose(ui, controller);
}

// general XWindow expose callback, 
static void _expose(const gx_lunaUI * const ui) {
...
static void controller_expose(const gx_lunaUI * const ui,  const gx_controller * const control) {
	cairo_push_group (ui->cr);
...
static void reset_request(gx_lunaUI * const ui) {
...
static void send_controller_event(gx_lunaUI * const ui, int controller) {
...
static void check_value_changed(gx_lunaUI * const ui, int i, const float * const value) {
...
static void check_is_active(gx_lunaUI * const ui, int i, bool set) {
...
static bool aligned(int x, int y, const gx_controller * const control, const gx_lunaUI * const ui) {
...
static bool get_active_ctl_num(gx_lunaUI * const ui, int *num) {
	bool ret = false;
	for (int i=0;i<CONTROLS;i++) {
...
static bool get_active_controller_num( const gx_lunaUI * const ui, int *num) {
	for (int i=0;i<CONTROLS;i++) {
...
static void scroll_event(gx_lunaUI * const ui, int direction) {
	float value;
	int num;
...
static void enum_event(gx_lunaUI * const ui, int i) {
	float value;
...
static void motion_event(gx_lunaUI * const ui, double start_value, int m_y) {
...
static void set_key_value(gx_lunaUI * const ui, int set_value) {
	float value = 0.0;
	int num;
	if (get_active_controller_num(ui, &num)) {
...
static void key_event(gx_lunaUI * const ui, int direction) {
	float value;
	int num;
	if (get_active_controller_num(ui, &num)) {
		value = min(ui->controls[num].adj.max_value,max(ui->controls[num].adj.min_value, 
...
static void set_previous_controller_active(gx_lunaUI * const ui) {
	int num;
	if (get_active_controller_num(ui, &num)) {
...
static void set_next_controller_active(gx_lunaUI * const ui) {
	int num;
	if (get_active_controller_num(ui, &num)) {
		ui->controls[num].is_active = false;
		send_controller_event(ui, num);
...
static void get_last_active_controller(gx_lunaUI * const ui, bool set) {
	int num;
	if (get_active_controller_num(ui, &num)) {
...
static void event_handler(gx_lunaUI * const ui) {
	XEvent xev;
