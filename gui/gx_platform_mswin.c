...
void gx_gui_create_window_and_surface(gx_lunaUI *ui) {
	...
	static TCHAR szClassName[] = TEXT("gx_lunaUIClass");
	WNDCLASS wndclass = {0};
...
void gx_gui_destroy_main_window(gx_lunaUI *ui) {
	...
	UnregisterClass(TEXT("gx_lunaUIClass"), NULL);
}
...
void event_handler(gx_lunaUI *ui) {
}
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	gx_lunaUI *ui = (gx_lunaUI *)GetWindowLongPtr(hwnd, GWLP_USERDATA);
